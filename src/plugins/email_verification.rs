...
impl EmailVerificationPlugin {
    ...
    async fn handle_verify_email(&self, req: &AuthRequest, ctx: &AuthContext) -> AuthResult<AuthResponse> {
        ...
        let query_params = self.parse_query_string(req.path())?;
        ...
        let callback_url = query_params.get("callbackURL");
    ...
    fn parse_query_string(&self, path: &str) -> AuthResult<std::collections::HashMap<String, String>> {
        ...
        if let Some(query_start) = path.find('?') {
            ...
            for pair in query.split('&') {
                if let Some(eq_pos) = pair.find('=') {
                    ...
                    let decoded_value = value.replace("%20", " ").replace("%40", "@");
